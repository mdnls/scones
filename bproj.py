...
def parse_args_and_config():
    ...
    args.log_path = os.path.join('baryproj', args.exp, 'logs', args.doc)

    # parse config file
    with open(os.path.join('baryproj/configs', args.config), 'r') as f:
        config = yaml.load(f)
    new_config = dict2namespace(config)

    tb_path = os.path.join('baryproj', args.exp, 'tensorboard', args.doc)
    ...
    if not args.sample and not args.fast_fid:
        ...
        if args.sample:
            ...
            os.makedirs(os.path.join('ncsn', args.exp, 'fid_samples'), exist_ok=True)
            args.image_folder = os.path.join('ncsn', args.exp, 'fid_samples', args.image_folder)
            if not os.path.exists(args.image_folder):
                os.makedirs(args.image_folder)
            else:
                overwrite = False
